...
export default function JobsPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      {/* ── Applications Tab (Archer Dashboard) ──────── */}
      {tab === 'applications' && <ArcherApplicationsTab onBrowseJobs={() => setTab('discover')} userPlan={userPlan} />}
    </div>
  );
}

// ── Archer Applications Tab Component ─────────────────────
...
function ArcherApplicationsTab({ onBrowseJobs, userPlan }: { onBrowseJobs: () => void; userPlan: PlanTier }) {
  ...
  const archerAvailable = isAgentAvailable('archer', userPlan);
  ...
  useEffect(() => {
    fetchApplications(statusFilter, appPage);
  }, [statusFilter, appPage, fetchApplications]);
  ...
  // ── Empty state ──
  if (!loading && apps.length === 0 && statusFilter === 'all') {
    return (
        ...
        {archerAvailable ? (
          <>
            <p className="text-sm text-white/40 mb-6 max-w-md mx-auto">
              Archer hasn&apos;t sent any applications yet. Run the full auto-apply pipeline from your dashboard, or discover jobs to build your list.
            </p>
            <div className="flex gap-3 justify-center">
              <Link href="/dashboard" className="btn-primary text-sm px-6 py-2.5 flex items-center gap-2">
                <Play className="w-4 h-4" /> Run Pipeline
              </Link>
              <button onClick={onBrowseJobs} className="btn-secondary text-sm px-6 py-2.5 flex items-center gap-2">
          ...
          <>
            <p className="text-sm text-white/40 mb-6 max-w-md mx-auto">
              Archer auto-applies to jobs on your behalf. Upgrade to the Pro plan or higher to unlock automated applications.
            </p>
            <Link href="/pricing" className="btn-primary text-sm px-6 py-2.5 inline-flex items-center gap-2">
                      ...
                      <div className="mt-4 pt-4 border-t border-white/5 space-y-3">
                        ...
                        {app.coverLetter && (
                          <div className="p-3 rounded-xl bg-white/[0.02] border border-white/5">
                            <p className="text-[10px] font-semibold text-white/30 uppercase tracking-wider mb-1.5">Cover Letter</p>
