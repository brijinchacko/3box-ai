  ...
  try {
    ...
    sendWelcomeEmail(email, name).then(() => {
      prisma.emailLog.create({
        data: { userId: user.id, type: 'WELCOME', subject: 'Welcome to 3BOX AI', to: email, status: 'sent' },
      }).catch(() => {});
    }).catch((err: any) => {
      console.error('[Register] Failed to send welcome email:', err);
      prisma.emailLog.create({
        data: { userId: user.id, type: 'WELCOME', subject: 'Welcome to 3BOX AI', to: email, status: 'failed' },
      }).catch(() => {});
    });
