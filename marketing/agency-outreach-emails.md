# 3BOX AI — Placement Agency & Training Institute Outreach Emails

---

## Email 1: Placement Agencies (India)

**Subject:** Partnership: AI career tools for your candidates — 3BOX AI

**To:** Placement agency managers, recruitment consultants

---

Hi [Name],

I'm Brijin from 3BOX AI — an AI-powered career platform that helps job seekers with skill assessment, resume building, career planning, and job matching.

I'm reaching out because I think our platform could add real value to your candidates:

**What 3BOX AI offers your candidates:**
- AI Skill Assessment — Identifies exact skill gaps for target roles
- AI Resume Builder — Creates ATS-optimized resumes in minutes
- Career Plan — Personalized roadmaps with milestones and projects
- Learning Paths — Curated courses to fill skill gaps fast
- Interview Prep — AI-generated practice questions with feedback

**Why partner with us:**
- Candidates who use our tools are better prepared = higher placement rates
- ATS-optimized resumes = more candidates clearing initial screening
- Skill assessment data helps you match candidates to roles more accurately
- We offer bulk/institutional pricing for agencies

**Current pricing:**
- Individual: Starts at ₹399/mo
- Agency/Bulk: Custom pricing (significant discounts for 50+ candidates)

Would you be open to a 15-minute call to explore how 3BOX AI could help improve your placement rates?

I'm happy to set up free trial accounts for your team to evaluate the platform.

Best regards,
Brijin Chacko
Founder, 3BOX AI
https://3box.ai
brijin@3box.ai

---

## Email 2: Training Institutes / Bootcamps (India)

**Subject:** Free AI career tools for your students — Partnership inquiry

---

Hi [Name],

I'm Brijin, founder of 3BOX AI (https://3box.ai) — an AI career platform that helps people go from skill assessment to job placement.

I noticed [Institute Name] trains students in [specific domain — e.g., software development, data science, digital marketing]. I think we could add value to your students' outcomes.

**What we offer for training institutes:**
1. **AI Skill Assessment** — Pre and post-training assessments to measure skill improvement
2. **Career Plan Generator** — Personalized career roadmaps based on each student's assessment
3. **AI Resume Builder** — Students graduate with professional, ATS-optimized resumes
4. **Portfolio Builder** — Students showcase course projects in a shareable portfolio
5. **Job Matching** — Connect graduates to relevant job openings

**Partnership model options:**
- **Free Tier Access** — Your students use our free tools at no cost
- **Institutional License** — Discounted Starter/Pro access for all enrolled students
- **White-Label** — Co-branded version for your institute (Enterprise tier)
- **Revenue Share** — Earn commission on student upgrades

**The business case:**
- Students with better job outcomes = better institute reviews = more enrollments
- Assessment data gives you measurable learning outcomes
- AI resume builder saves your placement team 10+ hours/week
- Portfolio URLs make your institute's graduates stand out

Would you be interested in a demo? I can show you exactly how our platform fits into your student journey.

Happy to start with a free pilot for one batch of students.

Best regards,
Brijin Chacko
Founder, 3BOX AI
https://3box.ai

---

## Email 3: HR / Corporate Training

**Subject:** AI-powered skill assessment and career development for your employees

---

Hi [Name],

I'm reaching out from 3BOX AI — an AI career platform that combines skill assessment, personalized learning paths, and career planning.

While we started as a job seeker tool, several companies have asked us about using 3BOX AI for internal employee development. Here's what we can offer:

**For your L&D / HR team:**
- **Skill Gap Analysis** — AI assessments identify exact skill gaps across your team
- **Personalized Learning Paths** — Each employee gets a curated development plan
- **Progress Tracking** — Dashboard showing team skill development over time
- **Career Pathing** — Help employees see growth opportunities within your organization
- **Resume/Profile Building** — For internal mobility and promotion discussions

**Benefits:**
- Reduce external hiring costs by developing existing talent
- Data-driven L&D decisions (not guesswork)
- Employees feel invested in = better retention
- Measurable skill improvement across teams

**Pricing:** Custom enterprise packages based on team size.

Would a 20-minute demo be useful? I can show you real examples of how the skill assessment and learning path features work.

Best regards,
Brijin Chacko
Founder, 3BOX AI
https://3box.ai

---

## Email 4: Career Counselors / Coaches (Individual)

**Subject:** Enhance your coaching with AI-powered career tools

---

Hi [Name],

I'm Brijin from 3BOX AI. I build AI career tools, and I think our platform could complement your coaching practice.

**How 3BOX AI helps career coaches:**
- **Pre-session Assessment:** Send clients to 3BOX AI for a skill assessment before coaching sessions. You get data-driven insights instead of relying on self-reporting.
- **Resume Optimization:** Clients build ATS-optimized resumes between sessions, freeing your time for higher-value coaching.
- **Learning Path:** Between your sessions, clients follow AI-curated learning plans aligned with their career goals.
- **Progress Tracking:** Monitor client progress through the dashboard — skills improving, milestones completed, portfolio growing.

**Our coach-friendly model:**
- You keep coaching — we handle the tools
- Clients pay 3BOX AI separately (you earn affiliate commission)
- Or you bundle it into your coaching package at wholesale pricing

**What coaches have told us:**
"I used to spend 30 minutes every session reviewing resumes. Now clients show up with AI-optimized resumes, and we can focus on strategy." — Career Coach, Mumbai

Would you be open to trying the platform? I'll set up a free Pro account for you to evaluate.

Best regards,
Brijin Chacko
Founder, 3BOX AI
https://3box.ai

---

## Outreach Strategy

### Target List (India Focus)

**Placement Agencies:**
1. TeamLease Services
2. Quess Corp
3. ABC Consultants
4. Randstad India
5. Michael Page India
6. Naukri/InfoEdge recruitment partners
7. Local city-specific agencies (Bangalore, Mumbai, Delhi, Hyderabad, Chennai)

**Training Institutes:**
1. upGrad
2. Simplilearn
3. Great Learning
4. NIIT
5. Imarticus Learning
6. Newton School
7. Masai School
8. Coding Ninjas
9. Local bootcamps and training centers

**Career Coaching Firms:**
1. Locus (career counseling)
2. iDreamCareer
3. Mindler
4. Individual coaches on LinkedIn (search "career coach India")

### Outreach Sequence
1. **Day 1:** Send initial email (above templates)
2. **Day 4:** Follow-up if no response: "Hi [Name], following up on my earlier email about 3BOX AI. Would a quick 10-minute call work this week?"
3. **Day 8:** Value-add follow-up: Share a relevant blog post or industry insight
4. **Day 14:** Final follow-up: "Last note from me — if the timing isn't right now, I'd love to stay connected for the future."
5. **Move on** if no response after 14 days

### Metrics to Track
- Emails sent per week
- Response rate (target: 15-20%)
- Calls booked (target: 5% of emails)
- Partnerships signed
- Students/candidates onboarded through partnerships
